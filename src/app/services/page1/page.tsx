...
const PlanJourney = () => {
  ...
  const handleTripTypeChange = (value: "single" | "return" | "multi") => {
    ...
    if (value === "multi" && multiStops.length === 0) {
      // automatically add one stopcard when switching to multi stop
      setMultiStops([{ location: "", date: "" }]);
    }
    if (value !== "multi") {
      setMultiStops([]); // clear stops if not multi
    }
  };

  return (
    <section className="w-full max-w-[1760px] mx-auto mt-[75px] 2xl:max-h-[877px] px-4 sm:px-6 md:px-4 2xl:px-[2px] bg-white">
      {/* Main Content */}
            ...
            <summary className="flex justify-between items-center py-4 cursor-pointer select-none list-none">
              ...
              <div className="flex items-center gap-2 sm:gap-4">
                <select
                  value={tripType}
                  onChange={(e) =>
                    handleTripTypeChange(e.target.value as "single" | "return" | "multi")
                  }
